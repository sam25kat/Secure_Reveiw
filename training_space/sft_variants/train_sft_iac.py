...
# ── Config ────────────────────────────────────────────────────────────────────
ENV_URL          = os.getenv("ENV_URL", "https://sam25kat-securereview-env-iac.hf.space")
ENV_REPO_ID      = os.getenv("ENV_REPO_ID", "sam25kat/securereview-env-iac")
TASK_ID          = "iac_review"
SCENARIO_FOLDER  = "iac"               # subdir under app/tasks/scenarios/
...
SYSTEM_PROMPT = """You are a senior security engineer reviewing infrastructure-as-code for misconfigurations.

Identify ALL security issues including:
- Public S3 buckets, open security groups (0.0.0.0/0)
- Hardcoded secrets, plaintext passwords
- Disabled encryption, missing TLS
- Privilege escalation (overpermissive IAM, root containers, --privileged)
- Missing logging/monitoring, public RDS

Output ONLY a valid JSON array of findings. Each finding must have:
  file, line (integer or null), rule_id (e.g. IAC-001), severity (critical/high/medium/low/info), description
...
def main():
    ...
    scenario_ids = [
        ...
        "iac_010", "iac_014", "iac_016", "iac_018",
        "iac_019", "iac_020",
